...
public class Statement
{
	...
	public static final String LOAD_BENCH__DATA = "SELECT `owner`, `created`, `title`, `background`, `archived` FROM `Benches` WHERE `benchId` = ?;";

	public static final String LOAD_BENCH__GET_USER_IDS = "SELECT `userId`, `level` FROM `Members` WHERE `benchId` = ?;";

	public static final String LOAD_BENCH__GET_BENCH_NODE_DATA = "SELECT `bNodeId`, `userId`, `title`, `x`, `y`, `width`, `height`, `archived` FROM `BenchNodes` WHERE `benchId` = ?;";

	public static final String LOAD_BENCH__GET_BENCH_NODE_CONTENT = "SELECT `lastEdit`, `type`, `content` FROM `BenchNodeContent` WHERE `bNodeId` = ?;";
